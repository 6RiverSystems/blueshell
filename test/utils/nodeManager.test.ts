	...
	describe('websocket api', function () {
		...
		beforeEach(function () {
			...
			removeBreakpointHelperStub = sinon
				.stub(RuntimeWrappers, 'removeBreakpointFromFunction')
				.callThrough();
			setBreakpointHelperStub = sinon
				.stub(RuntimeWrappers, 'setBreakpointOnFunctionCall')
				.callThrough();
		...
		describe('breakpoints', function () {
			...
			describe('client reconnect', function () {
				it('should send any existing breakpoints back to the client when the client reconnects', async function () {
					...
					const placeBreakpointRespObj: any = (
						await EventEmitter.once(clientMock, 'messageHandled')
					)[0];
